...
\label{fig:plot-carbon-cycle-timeseries-primary-1}
\end{figure*}
...
\label{fig:plot-carbon-cycle-anomaly-timeseries-1}
\end{figure*}
...
Monte Carlo filtering (MCF) is a type of sensitivity analysis that integrates well with our aim of constraining model behaviour given a large ensemble using History matching. The idea of MCF is to identify inputs that have an influence on whether the behaviour of the model falls within or outside acceptable bounds of behaviour. This is done by splitting samples of model behaviour into those that do or don't meet some constraint, and then examining differences between the cumulative distributions of the corresponding input variables.
...
\subsection{Direct constraint of ensemble members}
