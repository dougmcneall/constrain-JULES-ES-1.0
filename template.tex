...
Land surface models are an important tool in the study of climate change and its impacts, but their use can be hampered by uncertainties in good input parameter settings, and by errors in the models. Uncertainty quantification (UQ) methods have been developed to help constrain the choices of input parameter settings, to quantify and minimise model errors, and to asses the impact of these uncertainties on the use of models in policy advice. In this study we use UQ methods to constrain a large ensemble of JULES-ES-1.0, the new land surface component of the UK Earth system model UKESM. We use historical simulations of the land surface model, driven by reanalysis, to rule out ensemble members and corresponding input parameters that do not match modern observations of the land surface. As the land surface model is computationally expensive, we use a statistical proxy, termed an emulator, trained on a perturbed parameter ensemble of model runs, to rule out parts of parameter space where we do not have a model run. We take an iterated approach to constraining JULES, running an initial ensemble and training the emulator, before using it to choose a second wave of ensemble members consistent with historical land surface observations and expectations.

We provide an ensemble of JULES historical simulations that are statistically consistent with past land surface behaviour, and an associated constraint of the joint distribution of input parameters. We explore the resulting constraint on the behaviour of the carbon cycle during the historical period. We use the emulator to perform several types of sensitivity analysis, and identify the most (and least) important input parameters for controlling the output of JULES-ES-1.0. We provide quantitative estimates for the effect of varying each input parameter on a range of model outputs, and provide information on how parameters might be varied to improve the performance of the model, and eliminate model biases.

 We show that an emulator is a useful tool for providing modellers and policymakers information about the nature of the Earth system configuration of the land surface model JULES. We show that it is possible to constrain the input parameter space of a complex land surface model, even when there are a large number of uncertain input parameters. 
